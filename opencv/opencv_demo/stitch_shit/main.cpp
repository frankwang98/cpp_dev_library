...
int main(int argc, char* argv[])
{
	...
	cv::String filepath = "C:\\Users\\dev\\Desktop\\quanjing_save\\data\\*.jpg";
	glob(filepath, img_names, true);
	size_t num_images = img_names.size();
	cout << "共 " << num_images << " 张图片" << endl;
	...
	for (int i = 0; i < num_images; ++i)
	{
		...
		img = full_img;
		computeImageFeatures(finder, full_img, features[i]);	// 计算特征点
	...
	for (int img_idx = 0; img_idx < num_images; ++img_idx)
	{
		...
		if (abs(compose_scale - 1) > 1e-1)  //abs 
